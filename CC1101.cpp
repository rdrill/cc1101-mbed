...
static uint8_t cc1100_GFSK_38_4_kb[] = {
                    0x07,  // IOCFG2        GDO2 Output Pin Configuration
                    0x2E,  // IOCFG1        GDO1 Output Pin Configuration
                    0x80,  // IOCFG0        GDO0 Output Pin Configuration
                    0x07,  // FIFOTHR       RX FIFO and TX FIFO Thresholds
...
static uint8_t cc1100_GFSK_100_kb[] = {
                    0x07,  // IOCFG2        GDO2 Output Pin Configuration
                    0x2E,  // IOCFG1        GDO1 Output Pin Configuration
                    0x80,  // IOCFG0        GDO0 Output Pin Configuration
                    0x07,  // FIFOTHR       RX FIFO and TX FIFO Thresholds
...
static uint8_t cc1100_MSK_500_kb[] = {
                    0x07,  // IOCFG2        GDO2 Output Pin Configuration
                    0x2E,  // IOCFG1        GDO1 Output Pin Configuration
                    0x80,  // IOCFG0        GDO0 Output Pin Configuration
                    0x07,  // FIFOTHR       RX FIFO and TX FIFO Thresholds
...
static uint8_t cc1100_OOK_4_8_kb[] = {
...
uint8_t CC1100::begin()
{
    ...
    if(debug_level > 0){
      //  Serial.println("Init CC1100...");
    }

    //spi_begin();                          //inits SPI Interface
    reset();                              //CC1100 init reset

    spi_write_strobe(SFTX);wait_us(100);//flush the TX_fifo content
    spi_write_strobe(SFRX);wait_us(100);//flush the RX_fifo content
    ...
    if(debug_level > 0){
        dbs.printf("Partnum: 0x%02X \r\n", partnum);
        dbs.printf("Version: 0x%02X \r\n", version);
    }
...
//          }
      ...
      {
          dbs.printf("0x%02X ", Patable_verify[i]);
      }
      dbs.printf("\r\n");
    }
}
...
{
    if(debug_level > 0){
        dbs.printf("Mode: 0x%02X \r\n", MY_MODE);
        dbs.printf("Frequency: 0x%02X \r\n", MY_FREQ);
        dbs.printf("Channel: 0x%02X \r\n", MY_CHAN);
        dbs.printf("My_Addr: 0x%02X \r\n", MY_ADDR);
    }
}
//-------------------------------[end]------------------------------------------
...
uint8_t CC1100::rx_payload_burst(uint8_t rxbuffer[], uint8_t &pktlen)
{
    ...
    {
        if(debug_level > 0){
            dbs.printf("no bytes in RX buffer or RX Overflow!: %d \r\n", bytes_in_RXFIFO);
        }
        sidle();                                                  //set to IDLE
        spi_write_strobe(SFRX);wait_us(100);            //flush RX Buffer
...
uint8_t CC1100::sent_packet(uint8_t my_addr, uint8_t rx_addr, uint8_t *txbuffer,
                            uint8_t pktlen,  uint8_t tx_retries)
                            {
                                ...
                                {
                                    dbs.printf("ERROR: package size overflow\r\n");
                                    return FALSE;
                                }
                                ...
                                do                                                          //sent package out with retries
                                {
                                    ...
                                    while (ackWaitCounter < ACK_TIMEOUT )                   //wait for an acknowledge
                                    {
                                        if (packet_available() == TRUE)                     //if RF package received check package acknowge
                                        {
                                            ...
                                            wait_ms(1);                                       //delay to give receiver time
                                        }
                                    }
...
uint8_t CC1100::check_acknolage(uint8_t *rxbuffer, uint8_t pktlen, uint8_t sender, uint8_t my_addr)
{
    ...
    if((pktlen == 0x05 && \
        ...
        rxbuffer[3] == 'A' && rxbuffer[4] == 'c' && rxbuffer[5] == 'k')   //acknolage received!
        {
            ...
            if(debug_level > 0){
                dbs.printf("ACK! ");
                dbs.printf("RSSI:%i ",rssi_dbm);
                dbs.printf("LQI:0x%02X ",lqi);
                dbs.printf("CRC:0x%02X\r\n",crc);
            }
            return TRUE;
        }
    return FALSE;
...
uint8_t CC1100::wait_for_packet(uint8_t milliseconds)
{
    for(uint8_t i = 0; i < milliseconds; i++)
        {
            wait_ms(1);                 //delay till system has data available
            if (packet_available())
            {
                return TRUE;
            }
        }
    //dbs.printf(("no packet received!"));
    return FALSE;
}
//-------------------------------[end]------------------------------------------
...
void CC1100::set_myaddr(uint8_t addr)
{
 ...
 //   if(MY_ADDR != addr)
//    {
//        MY_ADDR = addr; //writes byte to eeprom
//    }
}
//-------------------------------[end]------------------------------------------
...
//    if(MY_CHAN != channel)
//    {
//        MY_CHAN = channel; //writes byte to eeprom
//    }
}
//-------------------------------[end]------------------------------------------
...
//    if(MY_MODE != mode)
//    {
//        MY_MODE = mode;            //sets the cc1100 frequency
//    }
}
//-------------------------------[end]------------------------------------------
...
//    if(MY_FREQ != ism_freq)
//    {
//        MY_FREQ = ism_freq; //selects the cc1100 frequency
//    }
}
//-------------------------------[end]------------------------------------------
...
void CC1100::set_freq(uint32_t freq)
{
    ...
    dbs.printf(("FREQ2:"));dbs.printf(req2);
    dbs.printf(("FREQ1:"));dbs.printf(req1);
    dbs.printf(("FREQ0:"));dbs.printf(req0);
    spi_write_register(FREQ2,freq2);                                         //stores the new freq setting for defined ISM band
    spi_write_register(FREQ1,freq1);
    spi_write_register(FREQ0,freq0);
}
...
//
//     if(debug_level > 0){
//         dbs.printf(("Temp:"));dbs.printf(ptemp_Arr[0]);dbs.printf(("."));Serial.println(ptemp_Arr[1]);
//     }
//
...
//    dbs.printf(itoa(val, buffer, 10)); //uart_puts replaced to Serial.print
//
//}/* uart_puti */
//|================================= END ======================================|
